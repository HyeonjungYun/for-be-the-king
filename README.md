...
측정 → 실패 → 원인 규명 → 수정 → 재측정의 전 과정을 날짜순으로 기록한 문서입니다.
**틀린 가설과 계측기 오류까지 지우지 않고 남겼습니다.**

| 절 | 내용 |
|---|---|
| § 0 ~ 0-1 | 30인 O(N²) 병목 — 인원 사다리 실측 · 패킷 병합 · 계측기 오차 2건 |
| § 0-2 | 전투 패킷 대역폭 — 그리고 이동 검증 구멍 |
| § 0-3 | 120인 — 층 분리. **1차 가설이 틀렸고 그 기록을 남겼다** |
| § 0-4 | DB 통합 후 재측정 — 영속화가 측정 재현성을 깬 사례 |
| § 0-5 | 커버리지 77% — 숫자보다 먼저 찾아낸 use-after-free |
| § 0-6 | 지급 트랜잭션 + 멱등성 — 8,000 동시 요청 · **제가 오독한 −38%** |
| § 0-7 | CI — 깨끗한 러너가 드러낸 저장소 무결성 문제 3건 |
| § 0-8 | 인증 서버 · 아이템 · 빌드 전수 검사 |
| **§ 9** | **현재 위치** — Phase 진척과 다음 액션 |

기술 결정의 근거는 [`docs/architecture/`](docs/architecture/) 의 ADR에 있습니다.
