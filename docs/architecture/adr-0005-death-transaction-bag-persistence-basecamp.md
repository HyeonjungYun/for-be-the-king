...
**Accepted (2026-09-12)** — 🔴 **구현 0%.** `.proto` 메시지와 `bags` 스키마, 부팅 시 만료
정리만 있고 사망 커밋 · 가방 드랍 · 루팅 · 베이스캠프 경로는 전부 M2 범위다.
`Accepted` 는 이 설계로 간다는 승인이며 구현 완료를 뜻하지 않는다.
...
2026-09-12 — `S_BAG_CONTENTS` 의 `skills` 포함 결정을 § ②-a 로 기록 (한 번 반대로
결정했다가 뒤집은 건)
...
### ②-a `S_BAG_CONTENTS` 는 스킬 정보를 포함한다

가방을 열면 안에 든 아이템의 **스킬까지 함께 내려보낸다.**

```protobuf
message S_BAG_CONTENTS {
    uint64 bag_id = 1;
    repeated ItemInstance items = 2;   // ItemInstance.skills 포함
    uint32 remaining_ms = 3;
}
```

> 🔴 **2026-09-12 기록. 한 번 반대로 결정했다가 뒤집었다.**
>
> 처음에는 "죽은 플레이어의 로드아웃을 감춘다"는 이유로 `skills` 를 비우자고 했다. 그것이
> 틀린 이유는 **가방 주인은 이미 죽었다는 것**이다. 정보 은닉(`equipment-skill-binding` B8)은
> **앞으로 싸울 상대**의 조합을 감추는 규칙이고, 시체의 유품에는 적용 대상이 없다.
>
> 반면 숨기면 **25슬롯 중 무엇을 가져갈지 고를 근거가 사라진다.** 남의 가방에서 무기 하나만
> 집어 들 때 그 무기가 어떤 스킬을 주는지 모르면 선택이 무작위가 되고, **"들고 나갈 것을
> 고르는 결정"이라는 익스트랙션의 핵심이 죽는다.**
>
> 스킬은 종류가 아니라 **인스턴스**에 붙으므로(B2), 같은 이름의 검 두 자루가 다른 스킬을
> 줄 수 있다. 그래서 아이템 목록만으로는 가치 판단이 불가능하다 — `skills` 가 있어야 한다.
