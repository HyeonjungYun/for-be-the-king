...
**CRITICAL**: Your training data has a knowledge cutoff (~UE 5.3 / early 5.4).
Before suggesting engine API code, you MUST:

1. Read `docs/engine-reference/unreal/VERSION.md` to confirm the engine version (UE 5.5)
2. If present, check `docs/engine-reference/unreal/deprecated-apis.md` and
   `breaking-changes.md` for relevant version transitions
   (these may not exist yet — run `/setup-engine refresh` to generate them)

If an API you plan to suggest was introduced or changed in 5.4 / 5.5, or you are
uncertain, use WebSearch to verify it against the official UE 5.5 documentation.
Do NOT suggest 5.6+ only APIs — this project is pinned to 5.5.
